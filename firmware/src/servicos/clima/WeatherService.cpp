...
void gerarMsgClima() {
  ...
  // Prioriza chuva para a mensagem do guarda-chuva
  if (climaAtual.weather_code >= 9 && climaAtual.weather_code <= 14) {
    strncpy(msgClima, "Leve guarda-chuva!", sizeof(msgClima) - 1);
  } 
  else {
    switch(tipoClima) {
      case 0:  // Muito frio (T < 10?)
      ...
      case 3:  // SOL (claro)
        ...
        break;
      case 4:  // TERMÔMETRO (quente, T >= 30)
        strncpy(msgClima, "Dia quente! Hidrate", sizeof(msgClima) - 1);
        break;
      default:
        strncpy(msgClima, "Tenha um bom dia!", sizeof(msgClima) - 1);
        break;
    }
  }
  msgClima[sizeof(msgClima) - 1] = '\0';
}
...
void pegarClima() {
  ...
  if (doc["daily"]["data"].size() > 0) {
    ...
    // Código do ícone (Meteosource usa "icon" para o ID numérico)
    if (daily["all_day"]["icon"].is<int>()) {
      climaAtual.weather_code = daily["all_day"]["icon"].as<int>();
    } else {
      // Fallback para o campo weather se icon falhar
      climaAtual.weather_code = 0;
    }
