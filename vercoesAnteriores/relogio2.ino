...
//
// CONFIGURACAO RAPIDA:
//   1. Mude WIFI_SSID e WIFI_PASSWORD na secao 3 abaixo
//   2. Se quiser clima, crie conta em openweathermap.org e mude apiClima
//   3. Mude cidadeClima para sua cidade
//   4. Grave no ESP32 e acesse o IP mostrado no display
////////////////////////////////////////////////////////////////////////////////
...
#define BACKUP_INTERVAL_MS 7200000UL  // 2 horas em ms (2*60*60*1000)
...
//
// Com 60 amostras a cada 5 segundos = 5 minutos de historico visivel.
#define PERF_HIST_SIZE 60
uint32_t perfHist[6][PERF_HIST_SIZE];  // [funcao][amostra] em microsegundos
int      perfHistPos  = 0;             // próxima posição a escrever
bool     perfHistFull = false;         // buffer já completou a primeira volta?
...
.tab{flex:1;padding:8px;text-align:center;background:#111118;border:1px solid #1a1a2e;border-radius:6px 6px 0 0;cursor:pointer;color:#64748b;font-size:0.78rem}
...
.tpanel.cgraf{white-space:normal;max-height:none;padding:8px}
.cgrid{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:8px}
.ccard{background:#111118;border:1px solid #1a1a2e;border-radius:8px;padding:4px;overflow:hidden}
.sbtn{display:block;width:100%;padding:9px;background:#1d4ed8;color:#fff;border:none;border-radius:7px;cursor:pointer;font-size:0.9rem;font-family:inherit;transition:background 0.2s}
.sbtn:hover{background:#2563eb}.sbtn:disabled{background:#334155;cursor:default}
</style>
</head>
<body>
...
<div class="tpanel cgraf" id="tp3">
<button class="sbtn" id="btnSalvar" onclick="forcaSalvar()">&#128190; Forcar Salvamento na Flash</button>
<button class="sbtn" style="background:#065f46" id="btnDownload" onclick="baixarBackup()">&#11015; Baixar Backup TXT</button>
<input type="file" id="arquivoBackup" accept=".txt" style="display:none" onchange="restaurarBackup(this)">
<button class="sbtn" style="background:#7c3aed" onclick="document.getElementById('arquivoBackup').click()">&#11014; Restaurar Backup TXT</button>
<div class="cgrid">
...
function drawChart(id,data,color,name,cur){
  ...
  // Usa o intervalo real dos dados + margem para centralizar a linha no grafico
  const spread=Math.max(mx-mn,1);
  const yMax=mx+spread*0.3;
  const yMin=Math.max(mn-spread*0.3,0);
  const rng=yMax-yMin||1;
  const px=i=>w*i/(data.length-1);
  const py=v=>h-10-(v-yMin)/rng*(h-20);
...
async function forcaSalvar(){
  const b=document.getElementById('btnSalvar');
  b.textContent='\u23F3 Salvando...';b.disabled=true;
  try{
    await fetch('/api/salvar');
    b.textContent='\u2705 Salvo!';
    setTimeout(()=>{b.textContent='\uD83D\uDCBE Forcar Salvamento na Flash';b.disabled=false;},2500);
  }catch(e){
    b.textContent='\u274C Erro';
    setTimeout(()=>{b.textContent='\uD83D\uDCBE Forcar Salvamento na Flash';b.disabled=false;},2500);
  }
}
// Baixa o historico de performance como arquivo .txt
async function baixarBackup(){
  try{
    const r=await fetch('/api/download_perf');
    const txt=await r.text();
    const blob=new Blob([txt],{type:'text/plain'});
    const url=URL.createObjectURL(blob);
    const a=document.createElement('a');
    a.href=url;a.download='backup_perf.txt';a.click();
    URL.revokeObjectURL(url);
  }catch(e){alert('Erro ao baixar backup');}
}
// Envia um arquivo .txt de backup para restaurar o historico
async function restaurarBackup(input){
  if(!input.files.length)return;
  const txt=await input.files[0].text();
  const r=await fetch('/api/upload_perf',{method:'POST',headers:{'Content-Type':'text/plain'},body:txt});
  alert(await r.text());
  input.value='';
}
function rA(d){
...
// Flag: true quando o usuario esta digitando em algum campo de alarme
// Evita que o auto-refresh apague o que o usuario digitou
let editando=false;
document.addEventListener('focusin',e=>{if(e.target.closest&&e.target.closest('#als'))editando=true;});
document.addEventListener('focusout',e=>{if(e.target.closest&&e.target.closest('#als')){setTimeout(()=>{if(!document.querySelector('#als input:focus,#als select:focus'))editando=false;},200);}});
async function gA(){
  try{
    const r=await fetch('/api/alarmes');const d=await r.json();
    // So reconstroi os alarmes se o usuario NAO estiver editando
    if(!editando)rA(d);
...
gS();gA();gSer();
setInterval(gS,3000);setInterval(gA,2000);setInterval(gSer,2000);
setInterval(()=>{if(document.getElementById('tp3').classList.contains('active'))gPerf();},5000);
</script>
</body>
</html>
)rawliteral";
