...
function drawChart(id,data,color,name,cur,label='us'){const c=document.getElementById(id);if(!c)return;fixCanvas(c);const ctx=c.getContext('2d'),w=c.width,h=c.height;ctx.fillStyle='#0d0d16';ctx.fillRect(0,0,w,h);ctx.strokeStyle='#1e2038';ctx.lineWidth=1;for(let i=0;i<4;i++){const y=10+(h-20)*i/3;ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(w,y);ctx.stroke();}if(!data||data.length<2){ctx.fillStyle=color;ctx.font='bold 10px monospace';ctx.fillText(name+': '+cur+label,4,14);ctx.fillStyle='#64748b';ctx.font='9px monospace';ctx.fillText('Aguardando amostras... (5min)',4,30);return;}const mx=Math.max(...data,1),mn=Math.min(...data,0),sp=Math.max(mx-mn,1),yMax=mx+sp*0.3,yMin=Math.max(mn-sp*0.3,0),rng=yMax-yMin||1,px=i=>w*i/(data.length-1),py=v=>h-10-(v-yMin)/rng*(h-20);const grad=ctx.createLinearGradient(0,0,0,h);grad.addColorStop(0,color+'44');grad.addColorStop(1,color+'08');ctx.fillStyle=grad;ctx.beginPath();data.forEach((v,i)=>i?ctx.lineTo(px(i),py(v)):ctx.moveTo(px(i),py(v)));ctx.lineTo(w,h-10);ctx.lineTo(0,h-10);ctx.closePath();ctx.fill();ctx.strokeStyle=color;ctx.lineWidth=2;ctx.shadowColor=color;ctx.shadowBlur=10;ctx.beginPath();data.forEach((v,i)=>i?ctx.lineTo(px(i),py(v)):ctx.moveTo(px(i),py(v)));ctx.stroke();ctx.shadowBlur=0;ctx.fillStyle=color;ctx.font='bold 10px monospace';ctx.fillText(name+': '+cur+label,4,14);ctx.fillStyle='#64748b';ctx.font='9px monospace';ctx.fillText('max:'+mx+`${label} `+'min:'+mn+`${label}`,4,26);ctx.textAlign='left';ctx.fillText(`(${label})`,4,h-2);ctx.textAlign='right';ctx.fillText(`(${label})`,w-4,26);ctx.textAlign='left';}
async function gPerf(){try{const r=await fetch('/api/perf');const d=await r.json();const pos=d.pos,full=d.full;const total=full?288:pos;const pct=Math.round(total/288*100);const el=document.getElementById('perfStatus');if(el)el.innerHTML=`<span style="color:#4ade80">${total}</span>/288 amostras (${pct}% de 24h) &bull; heap: <span style="color:#38bdf8">${(d.heap/1024).toFixed(1)}KB</span> livre &bull; CPU: <span style="color:#f97316">${d.cpu?d.cpu.toFixed(1):0}%</span>`;requestAnimationFrame(()=>{for(let f=0;f<7;f++){const raw=d.hist[f];let ord=full?[...raw.slice(pos),...raw.slice(0,pos)]:raw.slice(0,pos);const label=(f===4)?'\u00b0C':(f===6?'%':'us');const current=(f===4||f===6)?(d.atual[f]/10).toFixed(1):d.atual[f];drawChart('pc'+f,ord,PC[f],PN[f],current,label);}});}catch(e){console.error('[gPerf]',e);const el=document.getElementById('perfStatus');if(el)el.textContent='Erro ao carregar dados de performance';}}
...
async function gCfg(){try{const r=await fetch('/api/config');const d=await r.json();document.getElementById('cfgDht').value=d.dhtMs;document.getElementById('cfgClima').value=d.climaMs;document.getElementById('cfgBackup').value=d.backupMs;document.getElementById('cfgCidade').value=d.cidade||'curitiba';document.getElementById('wssid').value=d.ssid||'';}catch(e){}}async function salvarCfg(){const d=document.getElementById('cfgDht').value,c=document.getElementById('cfgClima').value,b=document.getElementById('cfgBackup').value,cid=encodeURIComponent(document.getElementById('cfgCidade').value);await fetch('/api/config/set?dht='+d+'&clima='+c+'&backup='+b+'&cidade='+cid);}
