...
// --- Display LCD I2C ---
#define LCD_ADDR      0x27  // Endereco I2C do LCD (verificar se eh 0x27 ou 0x3F)
#define LCD_COLS       16  // Colunas do LCD
#define LCD_ROWS        2  // Linhas do LCD (pode ser 2 ou 4)
...
LiquidCrystal_I2C lcd(LCD_ADDR, LCD_COLS, LCD_ROWS);         // Display LCD I2C
WebServer server(80);  // Servidor web na porta 80 (porta padrao HTTP)

// Variaveis para controlar qual display usar
bool oledAtivo = false;  // True se OLED iniciou com sucesso
bool lcdAtivo = false;   // True se LCD iniciou com sucesso
...
uint32_t perfHist[PERF_FUNCOES][PERF_HIST_SIZE];  // [funcao][amostra] serve para guardar os tempos
...
void oledMsg(const char* l1, const char* l2 = nullptr) {
  if (!oledAtivo) return; // Se OLED nao iniciou, nao faz nada
...
// Mostra mensagem no LCD I2C durante o boot
void lcdMsg(const char* l1, const char* l2 = nullptr) {
  if (!lcdAtivo) return; // Se LCD nao iniciou, nao faz nada
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print(l1);
  if (l2) {
...
void handleAlarmes() {
  ...
  char json[600];
  char* p = json;
  float dhtTemp = isnan(dhtTemperature) ? -127.0f : dhtTemperature; //isnan retorna true se o valor for NaN (Not a Number), ou seja, se a leitura do DHT falhou. Nesse caso, usamos -127.0f como valor de erro, pois é um valor improvável para temperatura e facilita a detecção de falhas no cliente.
...
void handleDownloadPerf() {
  ...
  for (int f = 0; f < PERF_FUNCOES; f++) {
    txt += String(nomes[f]) + ":";
    int total = perfHistFull ? PERF_HIST_SIZE : perfHistPos; //perfHistFull indica se o buffer já foi preenchido completamente pelo menos uma vez, ou seja, se perfHistPos já deu a volta completa. Se ainda não deu a volta completa, então os dados válidos estão apenas até perfHistPos.
    for (int i = 0; i < total; i++) {
      int idx = perfHistFull ? (perfHistPos + i) % PERF_HIST_SIZE : i; //perfHistPos aponta para a proxima posicao a ser escrita, entao o dado mais antigo esta em perfHistPos e o mais recente em perfHistPos-1
      if (i > 0) txt += ",";
...
bool bootStep() {
  ...
  switch (bootState) {
    ...
    case BOOT_DISPLAY:
      ...
      // Tenta iniciar LCD I2C
      // Detecta LCD por varredura de endereco I2C
      ...
      if (Wire.endTransmission() == 0) {
        ...
        lcd.backlight();
        lcdMsg("[1/8] LCD", "OK");
        Serial.println("LCD I2C inicializado com sucesso!");
      } else {
        Serial.println("LCD I2C nao encontrado!");
      }
    ...
    case BOOT_SPIFFS:
      ...
      SPIFFS.begin(true);
      // Deleta config WiFi anterior para resetar sempre com "Marcos"/"henrique"
      if (SPIFFS.exists("/wifi_config.txt")) {
        SPIFFS.remove("/wifi_config.txt");
        logPrintln("[BOOT] Arquivo WiFi anterior deletado - usando padrão 'Marcos/henrique'");
      }
      carregarPerfFlash(); carregarConfigParams();
      if (oledAtivo) oledMsg("[2/8] SPIFFS", "OK");
      if (lcdAtivo) lcdMsg("[2/8] SPIFFS", "OK");
      bootState = BOOT_RTC; bootStepTime = agora; break;
    case BOOT_RTC:
      if (agora - bootStepTime < BOOT_MSG_INTERVAL) break;
      if (rtc.begin() && rtc.lostPower()) rtc.adjust(DateTime(F(__DATE__), F(__TIME__)));
      if (oledAtivo) oledMsg("[3/8] RTC", "OK");
      if (lcdAtivo) lcdMsg("[3/8] RTC", "OK");
      bootState = BOOT_I2S; bootStepTime = agora; break;
    case BOOT_I2S:
      if (agora - bootStepTime < BOOT_MSG_INTERVAL) break;
      iniciarI2S();
      if (oledAtivo) oledMsg("[4/8] Audio", i2sIniciado ? "OK" : "Falhou");
      if (lcdAtivo) lcdMsg("[4/8] Audio", i2sIniciado ? "OK" : "Falhou");
      bootState = BOOT_BOTOES; bootStepTime = agora; break;
    case BOOT_BOTOES:
      if (agora - bootStepTime < BOOT_MSG_INTERVAL) break;
      pinMode(pinoButton1, INPUT_PULLUP); pinMode(pinoButton2, INPUT_PULLUP);
      if (oledAtivo) oledMsg("[5/8] Botoes", "OK");
      if (lcdAtivo) lcdMsg("[5/8] Botoes", "OK");
      bootState = BOOT_WIFI_START; bootStepTime = agora; break;
    case BOOT_WIFI_START:
      if (agora - bootStepTime < BOOT_MSG_INTERVAL) break;
      if (oledAtivo) oledMsg("[6/8] WiFi", wifiSSID);
      if (lcdAtivo) lcdMsg("[6/8] WiFi", wifiSSID);
    ...
    case BOOT_WIFI_RESULT:
      if (WiFi.status() == WL_CONNECTED) {
        ...
        if (lcdAtivo) {
          char apBuf[16];
          snprintf(apBuf, sizeof(apBuf), "%s", WiFi.softAPIP().toString().c_str());
          lcdMsg("[6/8] MODO AP", apBuf);
        }
    ...
    case BOOT_NTP:
      if (agora - bootStepTime < BOOT_MSG_INTERVAL) break;
      if (oledAtivo) oledMsg("[7/8] NTP", "Sincronizando");
      if (lcdAtivo) lcdMsg("[7/8] NTP", "Sinc");
      if (WiFi.status() == WL_CONNECTED) {
        configTime(gmtOffset_sec, daylightOffset_sec, ntpServer);
        struct tm ti;
        if (getLocalTime(&ti, 5000)) {
          rtc.adjust(DateTime(ti.tm_year + 1900, ti.tm_mon + 1, ti.tm_mday, ti.tm_hour, ti.tm_min, ti.tm_sec));
          if (oledAtivo) oledMsg("[7/8] NTP", "OK");
          if (lcdAtivo) lcdMsg("[7/8] NTP", "OK");
        } else {
          if (oledAtivo) oledMsg("[7/8] NTP", "Falhou");
          if (lcdAtivo) lcdMsg("[7/8] NTP", "Falhou");
        }
      } else {
        if (oledAtivo) oledMsg("[7/8] NTP", "Sem WiFi");
        if (lcdAtivo) lcdMsg("[7/8] NTP", "Sem WiFi");
      }
      bootState = BOOT_SERVER; bootStepTime = agora; break;
    case BOOT_SERVER:
      if (oledAtivo) oledMsg("[8/8] Server", "Iniciando");
      if (lcdAtivo) lcdMsg("[8/8] Server", "Init");
    ...
    case BOOT_DHT:
      ...
      if (lcdAtivo) {
        if (WiFi.status() == WL_CONNECTED) {
          char urlBuf[16];
          snprintf(urlBuf, sizeof(urlBuf), "%s", WiFi.localIP().toString().c_str());
          lcdMsg("OK SISTEMA", urlBuf);
        } else {
          lcdMsg("OK SISTEMA", "AP 192.168.4.1");
        }
      }
  ...
  if (estadoAtual == "RELOGIO") {
    ...
    if (precisaRedesenhar && estadoAtual == "RELOGIO") {
      ...
      if (lcdAtivo) {
        ...
        // Temperatura e umidade na primeira linha
        if (alarmeDisparo) {
          lcd.print(" ALM");
          lcd.print(alarmeDisparoIdx + 1);
        } else {
          lcd.print(" ");
          if (!isnan(dhtTemperature)) {
        ...
        // Segunda linha: WiFi/AP e umidade
        lcd.setCursor(0, 1);
        if (wifiConectado) {
          lcd.print("WiFi:");
          lcd.print(WiFi.RSSI());
        ...
        if (!isnan(dhtHumidity) && !alarmeDisparo) {
          lcd.setCursor(12, 1);
          if (dhtHumidity < 10) lcd.print("0");
          lcd.print((int)roundf(dhtHumidity));
          lcd.print("%");
        }
      }
    }
  ...
  // ---- MENU PRINCIPAL ----
    ...
    if (precisaRedesenhar && estadoAtual == "MENU") {
      ...
      if (lcdAtivo) {
        ...
        lcd.print("MENU");
        lcd.setCursor(0, 1);
        if (menuIndex == 0) lcd.print("> ALARMES");
        else if (menuIndex == 1) lcd.print("> HORA");
        else lcd.print("> SAIR");
      }
    }
  ...
  // ---- LISTA DE 3 ALARMES + SAIR ----
    ...
    if (precisaRedesenhar && estadoAtual == "LISTA_ALARMES") {
      ...
      if (lcdAtivo) {
        ...
        char linha[16];
        if (alarmeListaIdx < NUM_ALARMES) {
          snprintf(linha, sizeof(linha), "A%d: %02d:%02d %s",
            alarmeListaIdx + 1,
            alarmes[alarmeListaIdx].hora,
            alarmes[alarmeListaIdx].minuto,
            alarmes[alarmeListaIdx].ativo ? "ON" : "OFF");
        } else {
          snprintf(linha, sizeof(linha), "SAIR");
        }
        lcd.print(linha);
        ...
        if (alarmeListaIdx < NUM_ALARMES) {
          lcd.print("Mel: ");
          lcd.print(MELODIA_CURTO[alarmes[alarmeListaIdx].melodia]);
        } else {
  ...
  // ---- DETALHE DE UM ALARME ----
    ...
    if (precisaRedesenhar && estadoAtual == "DETALHE_ALARME") {
      ...
      if (lcdAtivo) {
        ...
        char linha[16];
        if (subMenuIndex == 0) {
          snprintf(linha, sizeof(linha), "> Status: %s", alarmes[alarmeListaIdx].ativo ? "ON" : "OFF");
        } else if (subMenuIndex == 1) {
          snprintf(linha, sizeof(linha), "> Hora: %02d:%02d", alarmes[alarmeListaIdx].hora, alarmes[alarmeListaIdx].minuto);
        } else if (subMenuIndex == 2) {
          snprintf(linha, sizeof(linha), "> Mel: %s", MELODIA_NOMES[alarmes[alarmeListaIdx].melodia]);
        } else {
  ...
  // ---- ESCOLHER MELODIA ----
    ...
    if (precisaRedesenhar) {
      ...
      if (lcdAtivo) {
        ...
        lcd.print("MELODIA");
        lcd.setCursor(0, 1);
        if (melodiaEscolha < TOTAL_MELODIAS) {
          lcd.print("> ");
          lcd.print(MELODIA_NOMES[melodiaEscolha]);
        }
      }
    }
  ...
  // ---- EDITAR HORA DO ALARME ----
    ...
    if (precisaRedesenhar) {
      ...
      if (lcdAtivo) {
        ...
        lcd.setCursor(0, 1);
        if (tempHora < 10) lcd.print("0");
        lcd.print(tempHora);
        lcd.print(":");
        if (tempMinuto < 10) lcd.print("0");
        lcd.print(tempMinuto);
      }
    }
